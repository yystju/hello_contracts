...
//
    ...
    validNFToken(_tokenId) override {
        address tokenOwner = idToOwner[_tokenId];
    ...
    function getApproved(uint256 _tokenId) external view validNFToken(_tokenId) override returns (address) {
    ...
    function _burn(uint256 _tokenId) internal validNFToken(_tokenId) virtual {
        address tokenOwner = idToOwner[_tokenId];
        _clearApproval(_tokenId);
...
contract NFTMdata is NFToken {
    ...
    function _getOwnerNFTCount(address _owner) internal view override returns (uint256) {
